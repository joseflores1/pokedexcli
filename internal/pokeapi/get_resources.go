...
func (c *Client) GetPokemonByLocationArea(locationArea string, cache *pokecache.Cache) (PokemonByLocationArea, error) {
	url := locationAreaURL + "/" + locationArea

	if value, ok := cache.Get(url); ok {
		fmt.Println("Retrieving data from cache!")
		resources := PokemonByLocationArea{}
		err := json.Unmarshal(value, &resources)
		if err != nil {
			return PokemonByLocationArea{}, fmt.Errorf("error unmarshalling data from cache: %w", err)
		}
		return resources, nil
	...
	req, err := http.NewRequest("GET", url, nil)
	if err != nil {
		return PokemonByLocationArea{}, fmt.Errorf("error making GET request: %w", err)
	}
	...
	res, err := c.httpClient.Do(req)
	if err != nil {
		return PokemonByLocationArea{}, fmt.Errorf("error getting response: %w", err)
	}

	defer res.Body.Close()

	if res.StatusCode > 299 {
		return PokemonByLocationArea{}, fmt.Errorf("error: StatusCode = %d", res.StatusCode) 
	}	

	data, err := io.ReadAll(res.Body)
	if err != nil {
		return PokemonByLocationArea{}, fmt.Errorf("error reading response data: %w", err)
	}
	cache.Add(url, data)
	...
	err = json.Unmarshal(data, &resources)
	if err != nil {
		return PokemonByLocationArea{}, fmt.Errorf("error unmarshalling response: %w", err)
	}
